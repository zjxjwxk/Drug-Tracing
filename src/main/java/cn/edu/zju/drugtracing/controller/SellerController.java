...
public class SellerController {
    ...
    @ApiOperation("上传药品销售平台信息（根据药品销售平台名称、药品销售平台类型（0-医院；1-药店；2-电商），上传药品销售平台信息）")
    @PostMapping("/setSeller")
    @ResponseBody
    public ServerResponse<String> setSeller(@ApiParam("药品销售平台名称") @RequestParam String sellerName,
                                            @ApiParam(value = "药品销售平台类型（0-医院；1-药店；2-电商）", example = "0") @RequestParam Integer sellerType) {
        return sellerService.setSeller(sellerName, sellerType);
    }
    ...
    public ServerResponse<String> setSellInfo(@ApiParam("药品小包编号（3bytes药品ID+3bytes大包ID+3bytes小包ID）") @RequestParam String packageID,
        ...
        return sellerService.setSellInfo(packageID, time, consumerAddr, price);
    }
}
