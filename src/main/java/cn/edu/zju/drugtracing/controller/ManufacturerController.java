...
public class ManufacturerController {
    ...
    @ApiOperation("上传药品生产企业信息（根据药品生产企业名称，上传药品生产企业信息）")
    @PostMapping("/setManufacturer")
    @ResponseBody
    public ServerResponse<String> setManufacturer(@ApiParam("药品生产企业名称") @RequestParam String manufacturerName) {
        return manufacturerService.setManufacturer(manufacturerName);
    }
    ...
    @ApiOperation("上传药方（根据药品编号、药品名称、药品原料名称（用逗号分割），上传药方）")
    ...
    public ServerResponse<String> setFormulation(@ApiParam("药品编号（3bytes药品ID）") @RequestParam String drugID,
                                         ...
                                         @ApiParam("药品原料名称（用逗号分割）") @RequestParam String material) {
        return manufacturerService.setFormulation(drugID, drugName, material);
    }

    @ApiOperation("上传包装信息（根据药品大包编号、包装时间（输入0则为当前时间）、药品原料编号（用逗号分割），上传包装信息）")
    ...
    public ServerResponse<String> setBoxInfo(@ApiParam("药品大包编号（3bytes药品ID+3bytes大包ID）") @RequestParam String boxID,
                                     ...
                                     @ApiParam("药品原料编号（用逗号分割）") @RequestParam String materialID) {
        return manufacturerService.setBoxInfo(boxID, time, materialID);
    }
}
